...
impl Encryptor {
    pub fn new() -> Self {
        ...
        for media_type in [MediaType::AUDIO, MediaType::VIDEO] {
            stats.insert(media_type, EncryptionStats::zero());
        }
    ...
    pub fn encrypt(
        ...
        for attempt in 1..=MAX_CIPHERTEXT_VALIDATION_RETRIES {
            ...
            attempts += 1;
            max_attempt = max_attempt.max(attempt as u32);

            let size = leb128_size(truncated_nonce as u64);
            let required_buffer_size = frame_size
            ...
            if encrypted_frame.len() < required_buffer_size {
                warn!(
                    "encryption failed, output buffer too small (need {}, have {})",
            ...
            let Ok(reconstructed_frame_size) = frame_processor.reconstruct_frame(encrypted_frame)
            else {
            ...
            if serialize_unencrypted_ranges(unencrypted_ranges, unencrypted_ranges_buffer)
                != ranges_size
            {
        ...
        stats.max_attempts = stats.max_attempts.max(max_attempt);
        if success {
            stats.successes += 1;
        } else {
            stats.failures += 1;
    ...
    pub fn get_max_ciphertext_byte_size(media_type: &MediaType, frame_size: usize) -> usize {
        let base_size = frame_size + SUPPLEMENTAL_BYTES + TRANSFORM_PADDING_BYTES;
        match media_type {
            MediaType::VIDEO => base_size + frame_size,
            MediaType::AUDIO => base_size,
        }
    }

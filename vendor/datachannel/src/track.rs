...
pub struct PacketizerInit {
    ...
    pub playout_delay_id: u8,
    pub playout_delay_min: u16,
    pub playout_delay_max: u16,
}

impl PacketizerInit {
    fn as_raw(&self) -> Result<(CString, sys::rtcPacketizerInit)> {
        let cname = CString::new(self.cname.clone())?;
        Ok((
            cname,
            sys::rtcPacketizerInit {
                ...
                nalSeparator: self.nal_separator as sys::rtcNalUnitSeparator,
                obuPacketization: sys::rtcObuPacketization_RTC_OBU_PACKETIZED_OBU,
                playoutDelayId: self.playout_delay_id,
                playoutDelayMin: self.playout_delay_min,
                playoutDelayMax: self.playout_delay_max,
            },
        ))
    }
}
    ...
    T: TrackHandler + Send,
{
    ...
    pub fn set_h264_packetizer(&mut self, init: &PacketizerInit) -> Result<()> {
        let (cname, mut raw) = init.as_raw()?;
        raw.cname = cname.as_ptr();
        check(unsafe { sys::rtcSetH264Packetizer(self.id, &raw) }).map(|_| ())
    }

    pub fn set_opus_packetizer(&mut self, init: &PacketizerInit) -> Result<()> {
        let (cname, mut raw) = init.as_raw()?;
        raw.cname = cname.as_ptr();
        check(unsafe { sys::rtcSetOpusPacketizer(self.id, &raw) }).map(|_| ())
    }

    pub fn chain_rtcp_sr_reporter(&mut self) -> Result<()> {
        check(unsafe { sys::rtcChainRtcpSrReporter(self.id) }).map(|_| ())
    }

    pub fn chain_rtcp_nack_responder(&mut self, max_stored_packets_count: u32) -> Result<()> {
        check(unsafe { sys::rtcChainRtcpNackResponder(self.id, max_stored_packets_count) })
            .map(|_| ())
    }

    pub fn current_rtp_timestamp(&self) -> Result<u32> {
